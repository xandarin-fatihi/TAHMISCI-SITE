...
(function () {
  ...
  const RECIPE_TOKEN_KEY = "tahmisci.backend.recipe.token";
  const RECIPE_ACCESS_KEY = "tahmisci.recipe.access";
  ...
  async function init() {
    ...
    if (await verifyBackendSession()) {
      ...
      safeSessionRemove(RECIPE_ACCESS_KEY);
      safeSessionRemove(RECIPE_TOKEN_KEY);
      applyRecipeAccessState();
    }
  }
  ...
  function cacheElements() {
    [
      ...
      "recipeModalActions", "recipeSectionTitle", "recipeModeIcon"
    ].forEach((id) => {
  ...
  async function validateRecipePassword(password) {
    ...
    if (!baseUrl || !window.fetch) {
      return { ok: false, message: "Backend bağlantısı gerekli." };
    }

    const backendResult = await loginBackend(password);
    if (backendResult.status === "ok") return { ok: true };
    if (backendResult.status === "limited") {
      return {
        ok: false,
        message: backendResult.message || "Cok fazla hatali deneme yapildi. Lutfen biraz sonra tekrar deneyin."
      };
    }
    if (backendResult.status === "denied") {
      return { ok: false, message: backendResult.message || "Backend sifresi hatali." };
    }
  ...
  async function loginBackend(password) {
    try {
      ...
      if (result.token) safeSessionSet(RECIPE_TOKEN_KEY, result.token);
      return { status: "ok" };
    } catch (error) {
  ...
  async function verifyBackendSession() {
    ...
    try {
      const response = await fetch(`${backendBaseUrl()}/api/recipe/me`, {
        credentials: "include",
  ...
  function bearerHeaders() {
    const token = safeSessionGet(RECIPE_TOKEN_KEY) || safeSessionGet(BACKEND_TOKEN_KEY);
  ...
  function renderRecipeModalPanel(panel) {
    ...
    if (els.recipeModalActions) {
      ...
      Array.from(els.recipeModalActions.querySelectorAll("[data-action]")).forEach((button) => {
        const isActive = button.dataset.action === `recipe-${activePanel}`;
        button.classList.toggle("is-active", isActive);
      });
    }

    if (els.recipeSectionTitle) {
      els.recipeSectionTitle.textContent = activePanel === "preparation" ? "Hazırlanışı" : "İçerik";
    }
