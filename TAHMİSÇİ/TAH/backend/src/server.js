...
app.get("/login.html", requireAdminHost, auth.redirectIfAdmin, sendProjectFile("admin/login.html"));
app.get("/password-reset", requireKnownHost, sendProjectFile("password-reset/index.html"));
app.get("/password-reset/", requireKnownHost, sendProjectFile("password-reset/index.html"));
...
}, express.static(path.join(projectRoot, "panel"), {
  ...staticOptions,
  index: "index.html"
}));
...
app.get("/recipe-data.js", (req, res, next) => {
  if (!isMainHost(req) && !isAdminHost(req)) return notFound(req, res);
  return auth.requireAdminPage(req, res, () => sendProjectFile("recipe-data.js")(req, res, next));
});
